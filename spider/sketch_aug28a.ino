/*
  ESP32-WROOM Ambient-House Generator — Single File .ino (UI + ZERO-NOISE)
  ------------------------------------------------------------------------
  - Stable KY-040 handling: single-edge ISR with debounce, no missed steps.
  - Zero noise floor: removed air/noise sources; percussive tick = decaying sine.
  - Clean output: correct I2S format for MAX98357A, startup fade-in, gentle levels.
  - DRAM-safe: compact delay/reverb buffers.
*/
...
// Optional: flip direction if your encoder feels inverted
#define ENC_DIR (+1)  // use -1 to invert
...
// -----------------------------
// Encoder state (stable, debounced)
// -----------------------------
...
static inline float frand() { return (xorshift32() & 0xFFFFFF) / 16777216.0f; }
static uint32_t expMsRand(float mean_ms) {
...
// -----------------------------
// Fast trig via wavetable (sine) — compact
// -----------------------------
static const int SINE_TBL_SZ = 1024;
...
// Delay — ~0.55s max (DRAM safe ~24KB)
static const int DELAY_MAX_SAMP = (int)(SR * 0.55f);
...
// Reverb — compact buffers (~20KB)
struct Comb { int16_t* buf; int len; int idx; float fb; LP1 damp; float z; };
struct Allpass { int16_t* buf; int len; int idx; float g; };
// Small sizes
static int16_t revBuf1[2003], revBuf2[2251], revBuf3[2503], revBuf4[2751];
static int16_t apBuf1[317], apBuf2[277];
static Comb revC[4];
...
struct Pad {
  ...
  float sawMix=0.20f;   // gentle
  float amp=0.30f;
};
struct Pluck {
  bool  on=false; float env=0, a=0.003f, d=0.25f;
  float phase=0, inc=0; HP1 hp; LP1 lp;
  float pan=0.5f;
};
static const int PLUCK_N = 4;
struct Perc {
  bool on=false; float env=0, a=0.0012f, d=0.020f;
  float phase=0, inc=0, gain=0.08f; // decaying sine tick (no noise)
};
struct Sub {
  float phase=0, inc=0, amp=0.12f;
};
...
struct Params {
  ...
  float tempo=104.0f;
  float volume=72.0f; // safe default
};
static Params P;
...
static void applyParamMapping() {
  ...
  float cutoff = expf(logf(400.0f) + (logf(7000.0f/400.0f))*moodN); // tamed a bit
  pad.cutoff = cutoff;
  pad.sawMix = 0.12f + 0.30f * moodN; if (pad.sawMix > 0.45f) pad.sawMix = 0.45f;
...
void IRAM_ATTR isr_encA() {
  uint32_t now = micros();
  if (now - encLastMicros < 500) return; // ~0.5ms debounce
  encLastMicros = now;
  int b = digitalRead(ENC_B);
  // On A rising, B state sets direction. Adjust ENC_DIR to taste.
  encDelta += (b ? -1 : +1) * ENC_DIR;
}
...
static void nextPage() { curPage = (curPage + 1) % PAGE_COUNT; }
...
static void startGesture() { gestureOn = true; gestEndMs = millis() + (3000u + (uint32_t)(frand()*3000.0f)); }
static void endGesture()   { gestureOn = false; }

// -----------------------------
// Pad/Sub/Pluck/Perc helpers (noise-free)
// -----------------------------
...
static void triggerPluck(float hz) {
  for (int i=0;i<PLUCK_N;i++){
    if (!plk[i].on) {
      plk[i].on = true; plk[i].env = 0.0f;
      plk[i].phase = 0.0f;
      plk[i].inc = TAU_F * hz * DT;
      plk[i].pan = 0.4f + 0.2f*frand();
      DSP::lp_set(plk[i].lp, min(6000.0f, pad.cutoff*1.1f), SR);
...
static void triggerPerc() {
  perc.on = true; perc.env = 0.0f; perc.phase = 0.0f;
  float f = 3000.0f + 3500.0f*(P.tone*0.01f) + 500.0f*frand(); // ~3–7kHz
  perc.inc = TAU_F * f * DT;
  perc.gain = 0.06f + 0.05f*(P.tone*0.01f); // subtle
}
...
static void audioBlock() {
  ...
  // Gesture boosts (subtle)
  float gestCut = gestureOn ? 1.18f : 1.0f;
  float gestSaw = gestureOn ? 1.08f : 1.0f;
  float gestSend= gestureOn ? 1.12f : 1.0f;

  // Coeff updates
  DSP::lp_set(pad.lp, pad.cutoff*gestCut, SR);
  DSP::lp_set(DSP::delayToneLP, 5500.0f*(0.7f+0.6f*(P.tone*0.01f)), SR);
  for (int i=0;i<4;i++){ DSP::revC[i].damp.a = 1.0f - expf(-TAU_F*(2000.0f + 6000.0f*revDamp)/SR); }

  // Delay time (3/16) clamped
  float spb = 60.0f / P.tempo;
  ...
  for (int n=0;n<BLOCK_SAMP;n++){
    ...
    // PAD (unison sine/saw blend, filtered)
    float padL=0, padR=0;
    pad.lfoPh += TAU_F * pad.lfoRate * DT; if (pad.lfoPh >= TAU_F) pad.lfoPh -= TAU_F;
    float ampLfo = 0.90f + 0.10f * (0.5f*(fast_sin(pad.lfoPh)+1.0f));
    for (int i=0;i<PAD_UNISON;i++){
      ...
      float s  = (1.0f - pad.sawMix*gestSaw)*s1 + (pad.sawMix*gestSaw)*DSP::softclip(saw*0.60f);
    ...
    // PERC (decaying sine tick)
    float pr = 0.0f;
    if (perc.on) {
      if (perc.env < 1.0f) perc.env += perc.a;
      else                 perc.env -= perc.d*DT*SR*0.001f;
      if (perc.env <= 0.0005f){ perc.on=false; perc.env=0.0f; }
      perc.phase += perc.inc; if (perc.phase >= TAU_F) perc.phase -= TAU_F;
      pr = sinf(perc.phase) * perc.env * perc.gain;
    }
    ...
    // FX
    float dryL=L, dryR=R;
    float send = delSend*gestSend + revMix*0.26f;
    float monoSend = (L+R)*0.5f * send;
    float dOut = DSP::delay_process(monoSend, delaySamp, delFB, 0.32f);
    float rIn  = monoSend + dOut*0.20f;
    float rOut = DSP::reverb_process(rIn);

    L = dryL + dOut*0.26f + rOut*revMix;
    R = dryR + dOut*0.26f + rOut*revMix;
    ...
    float master = (P.volume * 0.01f) * 1.02f * startFade;
...
// -----------------------------
// Encoder init (single ISR, rising edge only)
// -----------------------------
static void encInit() {
  pinMode(ENC_A, INPUT_PULLUP);
  pinMode(ENC_B, INPUT_PULLUP);
  pinMode(ENC_SW, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(ENC_A), isr_encA, RISING);
}
...
void setup() {
  ...
  P.mood=50; P.energy=40; P.space=35; P.tone=50; P.scaleIdx=SCALE_AEOLIAN; P.tempo=104; P.volume=72;
  applyParamMapping();
...
void loop() {
  ...
  // Debounced button (simple)
  static uint32_t lastBtnRead=0;
  uint32_t now = millis();
  if (now - lastBtnRead > 1) lastBtnRead = now; // tiny throttle
}

/* Notes:
   - If encoder direction feels reversed, set ENC_DIR to -1.
   - Noise floor is eliminated by removing air/white-noise sources and using sine ticks.
   - Output headroom + startup fade keep speakers safe; increase Volume page slowly if needed.
*/
